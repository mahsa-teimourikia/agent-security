...
def run_demo():
    ...
    log_scenario("Principal context substitution (Alice ID + Mallory Ctx)", "Denied (unauthenticated_principal)" if res_sub_alice.terminal_state == "blocked" and "unauthenticated_principal" in str(audit.events[-1].reason) else "Allowed")
    ...
    log_scenario("Workload context substitution (Doc ID + Agent Ctx)", "Denied (unauthenticated_workload)" if not res_sub_wl and audit.events[-1].reason == "unauthenticated_workload" else "Allowed")
