...
class TestIdentityPropagation:
    ...
    def test_invalid_ttl_rejected(self, delegation_service):
        issue_result = delegation_service.issue("alice", "research-agent", "document-service", {"read"}, {"doc-101"}, ttl_minutes=-5)
        assert issue_result.grant is None

    def test_delegation_cannot_escalate_operations(self, delegation_service):
        issue_result = delegation_service.issue("alice", "research-agent", "document-service", {"delete"}, {"doc-101"})
        assert issue_result.grant is None

    def test_delegation_cannot_escalate_resources(self, delegation_service):
        issue_result = delegation_service.issue("alice", "research-agent", "document-service", {"read"}, {"doc-secret"})
        assert issue_result.grant is None

    def test_cross_tenant_issuance(self, delegation_service):
        issue_result = delegation_service.issue("alice", "evil-agent", "document-service", {"read"}, {"doc-101"}) # alice (acme), evil (globex)
        assert issue_result.grant is None

    # --- Verification ---
    def test_expired_delegation(self, delegation_service, storage, clock, auth_storage):
        issue_result = delegation_service.issue("alice", "storage-service", "storage-service", {"read"}, {"doc-101"}, ttl_minutes=60)
    ...
    def test_tenant_mismatch(self, delegation_service):
        issue_result = delegation_service.issue("alice", "document-service", "document-service", {"read"}, {"doc-101"})
